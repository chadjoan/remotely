...
// Macro conversions: These shouldn't be extern(C)/@nogc, because they are only
//   used at compile-time anyways.
/+
...
string SSH_STRINGIFY(T)(T s) { return SSH_STRINGIFY(s); }
string SSH_TOSTRING(T)(T s) { return std.conv.to!string(s); }

/+
// I converted these but intentionally commented them out, because this version
// may not match the version actually in use on the system during compilation
// (ex: if this .d file gets shared onto a different machine than the one I made
// it on, or becomes out of date).

/* libssh version macros */
// #define SSH_VERSION_INT(a, b, c) ((a) << 16 | (b) << 8 | (c))
int SSH_VERSION_INT(int a, int b, int c) { return ((a) << 16 | (b) << 8 | (c)); }
...
string SSH_VERSION_DOT(int a, int b, int c) {
	return
		...
		std.conv.to!string(c);
}

//#define SSH_VERSION(a, b, c) SSH_VERSION_DOT(a, b, c)
string SSH_VERSION(int a, int b, int c) { return SSH_VERSION_DOT(a,b,c); }

/* libssh version */
enum LIBSSH_VERSION_MAJOR = 0;
enum LIBSSH_VERSION_MINOR = 7;
enum LIBSSH_VERSION_MICRO = 3;

enum LIBSSH_VERSION_INT = SSH_VERSION_INT(
	LIBSSH_VERSION_MAJOR,
	LIBSSH_VERSION_MINOR,
	LIBSSH_VERSION_MICRO);

enum LIBSSH_VERSION = SSH_VERSION(
	LIBSSH_VERSION_MAJOR,
	LIBSSH_VERSION_MINOR,
	LIBSSH_VERSION_MICRO);
+/
...
version(Windows)
{
	alias socket_t = core.sys.windows.winsock2.SOCKET;
}
version(Posix)
{
	alias socket_t = int;
...
enum SSH_LOG_DEBUG = 3;
/** Get trace output, packet information, ... */
enum SSH_LOG_TRACE = 4;
